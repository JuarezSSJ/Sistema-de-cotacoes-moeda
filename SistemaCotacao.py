...
#cores
background_Titulos = "#0f2f86"
background_Janela = "#6da3e6"
...
label_cotacao_unid = tk.Label(
    text="Cotações de Moedas", bg= background_Titulos, foreground="white", borderwidth=2, relief="solid")
...
label_selecionar_moeda = tk.Label(
    text="Selecione a moeda que deseja consultar: ", anchor="e",bg = background_Janela)
...
label_selecionar_data = tk.Label(
    text="Selecione a data que deseja consultar a cotação: ", anchor="e",bg = background_Janela)
...
label_cotacao_unid = tk.Label(text="",bg = background_Janela)
label_cotacao_unid.grid(row=3, column=0, columnspan=2, sticky="nsew")


botao_pg_cotacao = tk.Button(
    text="Pegar Cotação", command=pegar_cotacao, borderwidth=1, relief="solid", bg=background_Titulos, foreground="white")
...
label_cotacao_multi = tk.Label(text="Cotações de Multiplas Moedas",
                               bg=background_Titulos, foreground="white", borderwidth=2, relief="solid")
label_cotacao_multi.grid(row=4, column=0, columnspan=3,
                         padx=10, pady=10, sticky="nsew")
var_caminhoarquivo = tk.StringVar()

label_selecionar_arq = tk.Label(
    text="Selecione um arquivo em Execel com as Moedas na Coluna A: ",bg = background_Janela)
label_selecionar_arq.grid(row=5, column=0, columnspan=2, sticky="nsew")

botao_selecionar_arq = tk.Button(
    text="Clique para Selecionar", command=selecionar_arq, borderwidth=1, relief="solid", bg=background_Titulos, foreground="white")
botao_selecionar_arq.grid(row=5, column=2, padx=10, pady=10)

label_arquivo_selecionado = tk.Label(
    text="Nenhum Arquivo Selecionado", anchor="e", bg = background_Janela)
label_arquivo_selecionado.grid(
    row=6, column=0, columnspan=3, padx=10, pady=10, sticky="nsew")

label_data_inicial = tk.Label(text="Data Inicial: ", anchor="e", bg = background_Janela)
...
label_data_final = tk.Label(text="Data Final: ", anchor="e", bg = background_Janela)
...
botao_Atualizar_Cotacoes = tk.Button(
    text="Atualizar Cotações", command=atualizar_cotacoes, borderwidth=1, relief="solid", bg=background_Titulos, foreground="white")
botao_Atualizar_Cotacoes.grid(row=9, column=0, padx=10, pady=10, sticky="nsew")

label_arquivo_atualizado = tk.Label(text="", bg = background_Janela)
label_arquivo_atualizado.grid(
    row=9, column=1, columnspan=2, padx=10, pady=10, sticky="nsew")

botao_fechar = tk.Button(text="Fechar", command=janela.quit, bg=background_Titulos, foreground="white")
